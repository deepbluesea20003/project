...
# validation functions - naomi 

def name_validation(filename):
    global error_id
    error_id = 0
    ans = None
    if ".csv" in filename:
...
def downloadFiles():
    ...
    for filename in myFiles:
        ...
        if validateFile(filename):
            ...
            f = open("log.txt", "a")
            if error_id == 0:
                string = str(datetime.datetime.now()) + ' ERROR: ' + filename + ' IS EITHER BLANK OR HAS A BAD FILENAME \n'
                f.write(string)
            if error_id == 1:
